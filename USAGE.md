...
When the user opens the view, VS Code will then emit an activationEvent `onView:${viewId}` (e.g. `onView:sbts` for the example below). You can also control the visibility of the view by providing the `when` context value.
...
```typescript
vscode.window.registerTreeDataProvider('sbts', new DepNodeProvider());
```

See [sbts.ts](src/sbts.ts) for the implementation.
